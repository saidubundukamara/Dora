...
<script lang="ts">
    ...
    onMount(async () => {
        ...
        try {
            const businessId = 'your-business-id'; // replace with your actual business ID
...
>
    ...
    <!-- Display BirdEye reviews -->
    <section class="mx-auto max-w-6xl py-10">
        <h2 class="text-center text-3xl font-bold mb-10">Reviews</h2>
        {#if error}
            <p class="text-center text-red-500">{error}</p>
        {:else}
            {#each reviews as review}
                <div class="review">
                    <h3 class="text-xl font-bold">{review.author}</h3>
                    <p>{review.comment}</p>
